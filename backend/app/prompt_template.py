...
18. Rule 17 also covers exclusion-by-association, but ONLY for a FLAT sibling relationship — a numbered/lettered item with no exclusion language of its own, sitting in the SAME list, at the SAME numbering depth, as another item whose own heading contains "Exclusion(s)"/"Excluded" (e.g. "17.24 Tobacco" beside its own list-sibling "17.25 Additional Exclusions" — say excluded, not covered).
   This does NOT apply to a NESTED sub-heading. When a parent section states something IS covered/permitted (e.g. "2. Sub-limits Applicable to Cyber Extortion" says payments "are covered subject to a sub-limit..."), and a sub-heading UNDER that same parent narrows it to specific carve-out cases (e.g. "2.2 Excluded Extortion Scenarios" listing only insider misconduct or an unpatched system), that sub-heading is a narrow EXCEPTION to the parent's general rule, not a reclassification of it. Keep the parent's own covered/permitted statement as its own point, stated with its real conditions (sub-limit %, pre-approval, etc.), and — only if the exception itself is relevant to the question — add the carve-out as a SEPARATE point naming exactly which scenarios it excludes. Never let a nested exception sub-heading invert the parent's own general statement into a blanket exclusion; a section that opens by saying something IS covered is never, by itself, evidence that it's excluded.
19. KNOWLEDGE BASE chunks are ordered by relevance - at least one point must be built from the first (best-matching) chunk when it directly answers the question; don't let less-relevant later chunks (sharing only a keyword) crowd it out entirely. This matters most when chunks DISAGREE IN POLARITY: if the first (best-matching) chunk states something IS covered/permitted and a much-lower-ranked chunk further down only names a narrower excluded scenario, the first chunk's own covered/permitted statement is the answer to the question — lead with it, stated with its real conditions. Never build the whole answer around the lower-ranked narrower chunk instead, and never let it flip the top chunk's polarity; if both are genuinely relevant, state the top chunk's general rule first and the lower-ranked exception second, clearly scoped to what it actually excludes.
