...
# Forked from Layla 2026-08-17: the actual documents/metadata scheme for
# this deployment aren't known yet, so metadata-based retrieval filtering
# is off by default until that's defined. Single choke point —
# classify_query_policy_type() below always returning "general" while this
# is False makes build_metadata_filter()'s own guard (policy_type !=
# "general") naturally produce no filter at all, a true no-op rather than
# a workaround grafted on somewhere else. The insurance-specific vocab
# below (_POLICY_TYPE_HINTS etc.) stays in the file but is meaningless
# data for an unknown future domain — not worth deleting the plumbing
# around it, just confirming it never fires while this flag is off. Flip
# to true and replace that vocab once the real domain is known.
ENABLE_METADATA_FILTERING = os.getenv("ENABLE_METADATA_FILTERING", "false").strip().lower() == "true"
...
def tag_document(
    ...
    *,
    ...
    if not ENABLE_METADATA_FILTERING:
        # Skip the insurance-vocabulary regex/LLM scoring below entirely —
        # query-time filtering is already off (classify_query_policy_type),
        # so these tags would never affect retrieval anyway; this just
        # also saves the LLM refinement calls' latency/cost during
        # ingestion, and keeps the admin panel's document list from
        # showing cosmetically wrong insurance-flavored tags on documents
        # that aren't insurance-related at all.
        return {
            "doc_type": doc_type,
            "insurer": "UNKNOWN",
            "policy_type": "general",
            "insurer_confidence": 0.0,
            "policy_type_confidence": 0.0,
            "all_insurers": [],
            "all_policy_types": [],
        }
...
def classify_candidate_type(
    ...
    LLM-only now (2026-09-10, ported from Layla) — the cheap keyword-
    overlap pre-check this function used to run before ever asking the
    LLM has been removed entirely, not just made skippable. User's
    explicit direction: the only thing that decides policy_type (official
    OR candidate) is the LLM, full stop. Confirmed live on Layla as a
    real, damaging bug, not just a theoretical risk: uploading a
    genuinely new "fine art & collectibles insurance" document, 2 of 5
    sections cheap-matched an EXISTING unrelated candidate before the LLM
    ever got a chance to independently guess correctly like the other 3
    sections did. The skip_cheap_match parameter (previously opt-in, used
    only for bulk backfills — see the module's own history for why
    keyword-overlap "runs away" at exactly that scale) is now the
    unconditional default; kept as a parameter rather than deleted
    outright only so existing call sites don't need touching, not because
    the non-default value is still expected to be used anywhere.
    Returns None (no candidate at all) if the model's answer normalizes
    to a degenerate non-answer ("general", "unclear", etc.) — a real
    absence of a guess, not an empty-string label. Returns None
    immediately, with no keyword fallback of any kind, when no LLM is
    available.
    """
