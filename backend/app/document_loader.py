...
def _load_youtube(url: str) -> list[Document]:
    ...
    # SentenceSemanticChunker (rag.py / api.py) will do all chunking — it
    # falls back to fixed word-window pseudo-sentences for punctuation-free
    # content like auto-generated transcripts, and finds topic-shift
    # boundaries across the full text. Pre-splitting here with fixed word
    # windows would prevent it from detecting any boundary that straddles
    # a word-cut, so this stays as one whole Document.
...
def _strip_repeating_page_furniture(page_texts: dict[int, str]) -> dict[int, str]:
    ...
    Frequency is a reliable signal here: a genuine, one-off line of body
    text essentially never repeats character-for-character across several
    DIFFERENT pages — even a paragraph that revisits the same topic
    rephrases itself. A running header/footer does exactly that, by
    ...
    # count > 2: genuine one-off content stays below this bar, repeating
    # page furniture clears it easily on any document long enough to have
    # a stable running header.
