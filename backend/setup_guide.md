...
- **The app** — runs locally in Docker on your machine (port 8511)
...
```
curl -X POST http://localhost:8511/upload \
...
```
curl http://localhost:8511/upload/<job_id>
```
...
```
curl -X POST http://localhost:8511/ask \
...
```
curl -X POST http://localhost:8511/ask-stream \
  -H "Content-Type: application/json" \
...
**Add a YouTube video:**
```
curl -X POST http://localhost:8511/upload-video \
  -H "Content-Type: application/json" \
...
**Add a webpage:**
```
curl -X POST http://localhost:8511/upload-webpage \
  -H "Content-Type: application/json" \
...
curl http://localhost:8511/docs

# Remove a specific document
curl -X DELETE "http://localhost:8511/docs/policy.pdf"

# Wipe everything and start fresh
curl -X DELETE http://localhost:8511/docs

# List uploaded videos
curl http://localhost:8511/videos

# Remove a video
curl -X DELETE "http://localhost:8511/videos/https://youtube.com/watch?v=..."
```
...
# Start the server
uvicorn app.main:app --host 0.0.0.0 --port 8511 --reload
```
