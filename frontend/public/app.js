/* =========================================================================
   Assistant — production chat frontend — app.js
   Forked from Layla (InsureHub-RAG) 2026-08-17.
   Single persistent chat window, no sidebar, no multi-conversation
   switching — matches the production client this was forked from
   (confirmed against the prior React client, frontend/src/App.tsx, which
   never had a conversation-history sidebar either). session_id is generated once
   and never rotates — even across "Clear chat" (see clearChat()) — so
   backend-side conversation context stays tied to the same session across
   a reset. "Clear chat" empties the visible thread client-side and resets
   server-side conversational state via API.resetConversation() on that
   SAME session_id; the full transcript is never deleted server-side.
   ========================================================================= */
...
// window.RAG_ASSISTANT_MOCK_MODE = true in a <script> before this file loads).
...
const MOCK_MODE = window.RAG_ASSISTANT_MOCK_MODE ?? false;
const API_BASE_URL = window.RAG_ASSISTANT_API_BASE_URL ?? '';

const STORAGE_KEY = 'rag_assistant_ui_state_v1';
...
const API = {
  ...
  // Resets server-side conversation state on the SAME session_id (never
  // deletes the session or its transcript — see clearChat()) so the
  // backend keeps seeing one continuous session per user across a
  // "Clear chat".
  async resetConversation(sessionId) {
    if (MOCK_MODE) return;
    try {
      await fetch(`${this.baseUrl}/conversation/reset/${sessionId}`, { method: 'POST' });
    } catch { /* best-effort — worst case stale conversational state lingers until the next reset */ }
  },
  ...
  async *streamAsk(query, sessionId, signal) {
    ...
    // FastAPI /ask-stream contract: body is {question, session_id};
    // response is plain streamed answer text with a final
    // '\n\n{"sources": [...], "done": true, ...}' JSON blob appended at
    // the end — not NDJSON/SSE. Same parsing already proven in the
    // production client this was forked from (Layla).
...
// Real HTML <table> for structured X-vs-Y comparison replies — table is
// {columns: [name, ...], rows: [{label, values: [...]}]}, sent by the RAG
// backend's comparison_table ui marker (multi_source_rag.py's
// _enforce_comparison_table_format) when a query asks to compare two
// retrieved topics. Wrapped in a horizontally-scrollable container since
// column names can be long and this renders inside a narrow chat bubble.
...
// reset did, landing on stale conversational/routing state. Confirmed live.
...
function buildMessageEl(m) {
  ...
  avatar.textContent = m.role === 'user' ? 'You' : 'A';
  ...
  try {
    for await (const evt of API.streamAsk(queryText, state.sessionId, activeAbortController.signal)) {
      if (evt.type === 'chunk') {
        ...
        // Structured X-vs-Y comparison reply — the chat bubble has no
        // markdown table renderer (a plain-text pipe table used to show up
        // as literal '|'/'---' characters), so the RAG backend sends the
        // comparison as structured data instead of text
        // (multi_source_rag.py's _enforce_comparison_table_format); attach
        // it to this message so buildMessageEl can render a real <table>
...
// the backend should keep seeing ONE continuous session per user, not a
// new session every time someone clicks Clear chat. Only the VISIBLE
// thread is erased client-side; nothing is deleted server-side —
// API.resetConversation() resets server-side RAG conversational memory on
// that SAME session_id instead.
//
